...
let custom = {
        ...
        return new Promise(function(resolve, reject) {
            ...
            let options = {
                files: [
                    ...
                    `/assets/css/theme/${GBL.DESIGN.THEME}/thync-manager-custom.css${ver_string}`,
                ],
                errorAfterType: "stop",
                callback: loadingEnd
            }
    ...
    process: {
        ...
        //     ///////////////////////////////////////////////////////////////////////////////////////////////////////////
        //     // 목록은 개별적으로 "MEASUREMENT.LIST.XXX" 설정한다.
        ...
        //     SOCKET_CONST.SET_FUNCTION(DEFAULT_FUNCTION, CUSTOM_FUNCTION);
        //     let initInfo = {
        ...
        init: async function() {
            ...
            if (GBL.SESSION_FIRST_CHECK === false) {
                if (GBL.ACCOUNT.IS_AUTH() === true) {
                    ...
                    // 추가 처리
                    let _r = await _checkPostAuth();
                    if (_r === `AUTH_FAIL`) {
                        return "AUTH_FAIL_AFTER";
                    }
                }
                else {
                    return "AUTH_FAIL";
            ...
            function _setHandlebarsHelper() {
                // 아래 영역에 코드 작성
                ///////////////////////////////////////////////////////////////////////////////////////////////////////
                Handlebars.registerHelper('KbToMb', function(kb) {
                    const mb = kb / 1024;
                    // 소수점 첫 번째 자리까지 표시
                    const roundedMb = mb.toFixed(1);
                    // 천 단위로 콤마 추가
                    const formattedMb = Number(roundedMb).toLocaleString();
                    return formattedMb;
                });
                ...
                // 장치 목록 분을 d/h/m으로 표시
                Handlebars.registerHelper('parsingForTotalUseTime', function(useTime, capital=null) {
                    if(useTime === 0) {
                        return "-";
                    }
                    if(capital) return Date.elapsedTimeForDurationSeconds("D_H_M", useTime * 60);
                    return Date.elapsedTimeForDurationSeconds("d_h_m", useTime * 60);
                })

                Handlebars.registerHelper('selectorToDomParsing', function(value) {
                    return custom.etc.selectorToDomParsing(value);
                });

                Handlebars.registerHelper('stringToSubStr', function(value, from, length) {
                    return value.substr(from, length);
                });

                Handlebars.registerHelper('iso8601ToDateString', function(string, type) {
                    if (string !== null) {
                        return new Date(string).toString(type);
                    }
                    else {
                        return `-`;
                    }
                ...
                Handlebars.registerHelper('setBackgroundForRise', function(object) {
                    if (object.recommendation_price > object.trade_price) {
                        return "background: #e0e0f3"
                    }
                    else if (object.recommendation_price < object.trade_price) {
                        return "background: #f3e8e8"
                    }
                    else {
                        return null
                    }
                ...
                Handlebars.registerHelper('setCommaForString', function(string) {
                    if (string === null || string === 0) {
                        return `-`
                    }
                    return Number.comma(Number(string))
                });
                ///////////////////////////////////////////////////////////////////////////////////////////////////////
            }

            async function _checkPostAuth() {
                // 아래 영역에 코드 작성
                ///////////////////////////////////////////////////////////////////////////////////////////////////////
                let response = await custom.etc.checkPostAuth(GBL.ACCOUNT.INFO.level, {isUse: true, msg: null});
                return response;
                ///////////////////////////////////////////////////////////////////////////////////////////////////////
            }
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        preForOutOfControllerScope: async function(domId = null, parameters = null) {
            ...
            // 아래 영역에 코드 작성(공통부분)
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////
            // refresh는 예외로 먼저 처리를 한다.
            if (GBL.WINDOW_HISTORY_STATE.URL.length === 0 || GBL.WINDOW_HISTORY_STATE.IS_CHANGE(true) === true || GBL.WINDOW_HISTORY_STATE.IS_SUB_CHANGE(true) === true) {
                // 페이지 새로고침 변수 초기화
    ...
    etc: {
        ...
        excelImport: function(file, callback = null) {
            const _reading = function() {
                const reader = new FileReader();
                reader.onload = async function (e) {
                    const data = await _parsing(e.target.result);
                    callback(data);
                };

                reader.readAsArrayBuffer(file);
            }
            const _parsing = async function(arrayBuffer) {
                const workbook = new ExcelJS.Workbook();
                await workbook.xlsx.load(arrayBuffer);
                const worksheet = workbook.getWorksheet(1); // 첫 번째 시트를 가져옵니다.

                const data = [];
                worksheet.eachRow({ includeEmpty: true }, function (row, rowNumber) {
                    const rowData = {};
                    row.eachCell({ includeEmpty: true }, function (cell, colNumber) {
                        rowData[`column${colNumber}`] = cell.value ? cell.value : null;
                    });
                    data.push(rowData);
                });
        ...
        // 추가 인증 관련 처리
        checkPostAuth: async function(userLevel, error = {isUse: false, msg: null}) {
            // 아래 영역에 코드 작성
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////
            const {CONST: USER_LEVEL_CONST} = await import(`/js/custom/constant/user-level.js${ver_string}`);
            let response = `OK`;
            if (userLevel < USER_LEVEL_CONST.LEVEL.CODE.SYSTEM_ADMIN) {
                response = `AUTH_FAIL`;
                if (error.isUse === true) {
                    error.msg = error.msg === null ? USER_LEVEL_CONST.LIMIT_MESSAGE : error.msg;
                    custom.etc.customToastForColor(error.msg, `bgRed`, `top-center`);
                }
            }
            return response;
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        selectorToDomParsing: function(value) {
    ...
    request: {
        ...
        api: function(url, parameter, onSuccessFunc = null, onSuccessParams = null, header = null, etcParams = {isFormData: false, httpStatusErrorFunction: null, requestType: `POST`}) {
            ...
            if(fakeApi === true) {
                ...
                if (etcParams.hasOwnProperty(`requestType`) === true) {
                    req.setType(etcParams.requestType);
                }
